...
import numpy as np
import json
from scripts.parse_testset import generate_features, cluster_samples, parse_row, process_line
...
def test_cluster_samples():
    ...
    assert np.array_equal(cluster_1, np.array([[1, 2], [1, 1]])) or np.array_equal(cluster_2, np.array([[1, 2], [1, 1]]))
